...
export class CommentsRepository  {
    async createComment (newComment: CommentsType) {
        const result = await CommentsModelClass.create(newComment)
        const comment = await CommentsModelClass.findOne({id: newComment.id}, {projection: {_id: 0, postId: 0}})
        return comment
    }
    async findComment (commentId: string): Promise<CommentsType | undefined | null> {
        const comment = await CommentsModelClass.findOne({id: commentId}, {projection: {_id: 0, postId: 0}})
        return comment
    }
    async findCommentWithPag(postId: string, pageSize:number, pageNumber:number){
        return CommentsModelClass.find({postId: postId}, {projection: {_id: 0, postId: 0}}).skip((pageNumber-1)*pageSize).limit(pageSize).lean()
    }
    ...
    async findUser(userId:string, commentId: string){
        return CommentsModelClass.findOne({userId: userId, id:commentId})
    }
    ...
    async updateCommentLikeStatus(user: string, commentId: string, likeStatus: string) {
        return CommentsModelClass.updateOne({id: commentId}, {$set: {likeStatus}})
    }
}